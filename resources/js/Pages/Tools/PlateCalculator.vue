        ...
        <div class="space-y-6">
            ...
            <GlassCard>
                <div class="p-6 space-y-6">
                    ...
                    <!-- Visualization -->
                    <div v-if="calculatedPlates.length > 0" class="mt-8 p-4 bg-white/5 rounded-xl overflow-x-auto">
                        <div class="flex items-center justify-center min-w-[300px] h-[200px] relative">
                            <!-- Bar -->
                            <div class="absolute w-full h-4 bg-gray-400 rounded-full z-0"></div>

                            <!-- Center Marker -->
                            <div class="absolute h-20 w-1 bg-gray-500 z-0"></div>

                            <!-- Left Side Plates -->
                            <div class="absolute left-[50%] flex flex-row-reverse items-center -translate-x-[calc(100%+10px)] gap-1">
                                <div
                                    v-for="(plate, index) in calculatedPlates"
                                    :key="`left-${index}`"
                                    class="flex items-center justify-center bg-red-500 text-xs font-bold text-white shadow-lg border-2 border-white/20"
                            ...
                            <!-- Right Side Plates -->
                            <div class="absolute left-[50%] flex items-center translate-x-[10px] gap-1">
                                <div
                                    v-for="(plate, index) in calculatedPlates"
                                    :key="`right-${index}`"
                                    class="flex items-center justify-center bg-red-500 text-xs font-bold text-white shadow-lg border-2 border-white/20"
                        ...
                        <div class="mt-4 text-center text-white/80">
                            <p>Total Weight: <span class="font-bold text-white">{{ actualWeight }}</span></p>
                            <p class="text-sm">Plates per side: {{ calculatedPlates.map(p => p.weight).join(', ') }}</p>
                        </div>
                    </div>
            ...
            <GlassCard>
                <div class="p-6 space-y-6">
                    <div class="flex justify-between items-center">
                        ...
                        <GlassButton @click="addingPlate = true" size="sm">
                            Add Plate
                        </GlassButton>
                    </div>

                    <div v-if="plates.length === 0" class="text-center text-white/50 py-8">
                        No plates in inventory. Add some to start calculating.
                    </div>

                    <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        <div v-for="plate in plates" :key="plate.id" class="relative group">
                            <div class="p-4 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition-colors">
                                ...
                                <button
                                    ...
                                    class="absolute top-2 right-2 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"
                                >
