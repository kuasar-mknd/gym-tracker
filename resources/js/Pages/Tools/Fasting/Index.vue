    ...
    <AuthenticatedLayout page-title="Suivi de Jeûne" show-back back-route="tools.index">
        ...
        <div class="space-y-6">
            ...
            <div v-if="activeFast" class="flex justify-center">
                 <GlassCard class="w-full max-w-md p-8 relative overflow-hidden text-center flex flex-col items-center">
                    <!-- Circular Progress -->
                    <div class="relative w-64 h-64 mb-6">
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                             <span class="text-4xl font-bold text-text-main font-mono">{{ formattedElapsedTime }}</span>
                             <span class="text-sm text-text-muted mt-2">{{ remainingTime }}</span>
                        </div>
                    </div>
            ...
            <!-- Start Fast Section -->
            <GlassCard v-else class="p-6 max-w-md mx-auto">
                <h3 class="text-lg font-semibold text-text-main mb-6">Démarrer un jeûne</h3>

                <form @submit.prevent="startFast" class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-text-muted mb-2">Type de jeûne</label>
                        <select
                            v-model="selectedType"
                            class="w-full rounded-xl border-white/10 bg-white/5 text-text-main focus:border-electric-orange focus:ring-electric-orange"
                        >
                    ...
                    <div>
                        <label class="block text-sm font-medium text-text-muted mb-2">Début</label>
            ...
            <!-- History Section -->
            <GlassCard class="p-6">
                <h3 class="text-lg font-semibold text-text-main mb-4">Historique</h3>
                 <div v-if="history.data.length === 0" class="text-text-muted text-center py-4">
                    Aucun historique de jeûne.
                </div>
                <div v-else class="space-y-3">
                    <div v-for="fast in history.data" :key="fast.id" class="flex items-center justify-between p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
                        <div>
                            <p class="text-text-main font-medium">{{ fast.type }}</p>
                            <p class="text-xs text-text-muted">
                                {{ formatDate(fast.start_time) }} - {{ fast.end_time ? formatDate(fast.end_time) : 'En cours' }}
                            </p>
                        </div>
                        <div class="text-right flex items-center gap-3">
                            <span class="text-text-main font-mono text-sm" v-if="fast.end_time">
                             ...
                             <button @click="deleteFast(fast.id)" class="text-red-400 hover:text-red-300 transition-colors">
