...
const icons = ['check_circle', 'fitness_center', 'water_drop', 'bedtime', 'restaurant', 'self_improvement', 'local_fire_department', 'bolt', 'directions_run', 'monitor_heart', 'spa', 'medication', 'local_cafe', 'no_drinks', 'savings', 'book']
const colors = ['bg-slate-500', 'bg-red-500', 'bg-orange-500', 'bg-amber-500', 'bg-green-500', 'bg-emerald-500', 'bg-teal-500', 'bg-cyan-500', 'bg-sky-500', 'bg-blue-500', 'bg-indigo-500', 'bg-violet-500', 'bg-purple-500', 'bg-fuchsia-500', 'bg-pink-500', 'bg-rose-500']
...
const toggleHabit = (habit, date) => {
    router.post(route('habits.toggle', habit.id), {
        date: date
    }, {
        preserveScroll: true,
        preserveState: true,
...
<template>
    ...
    <AuthenticatedLayout page-title="Habitudes">
                ...
                <div class="grid grid-cols-[1fr_repeat(7,minmax(32px,1fr))] sm:grid-cols-[200px_repeat(7,1fr)]">
                    ...
                    >
                        ...
                        <div
                            ...
                            :class="day.is_today ? 'text-accent-primary' : 'text-text-main'"
                        >
                            {{ day.day_num }}
                        </div>
                    </div>
                             ...
                             <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-[10px] text-text-muted">{{ getCompletionCount(habit) }}/{{ habit.goal_times_per_week }}</span>
                                    </div>
                                </div>
                             </div>
        ...
        <div v-if="showAddForm" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <GlassCard class="relative w-full max-w-lg shadow-2xl animate-scale-in" variant="solid">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-xl font-bold text-text-main">{{ editingHabit ? 'Modifier' : 'Nouvelle Habitude' }}</h3>
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    <GlassInput v-model="form.name" label="Nom" placeholder="Ex: Boire 2L d'eau" :error="form.errors.name" required />

                    <GlassInput v-model="form.description" label="Description (optionnel)" placeholder="Détails..." :error="form.errors.description" />
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button
                                ...
                                :class="[form.icon === icon ? 'border-accent-primary text-accent-primary bg-accent-primary/10' : 'border-transparent text-text-muted']"
                            >
