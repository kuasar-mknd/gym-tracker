...
const getCompletionCount = (habit) => {
    if (!props.weekDates || props.weekDates.length === 0) return 0

    // Filter logs to count only those in the current week
    const weekStart = props.weekDates[0].date
    const weekEnd = props.weekDates[props.weekDates.length - 1].date

    return habit.logs.filter((log) => log.date >= weekStart && log.date <= weekEnd).length
}
...
<template>
    ...
    <AuthenticatedLayout page-title="Habitudes">
        ...
        <div class="space-y-6">
            <!-- History Chart -->
            <div v-if="habits.length > 0" class="animate-slide-up">
