    ...
    <AuthenticatedLayout :page-title="part">
        ...
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold text-text-main">{{ part }}</h2>
        ...
        <div class="space-y-6">
            ...
            <GlassCard class="animate-slide-up">
                <h3 class="mb-4 font-display text-xs font-black uppercase tracking-[0.2em] text-purple-400">History</h3>
                 <BodyPartHistoryChart
                        ...
                        />
                         <div class="space-y-1">
                            <label class="block text-sm font-medium text-text-muted">Unit</label>
                             <div class="px-4 py-3 rounded-xl border border-white/10 bg-white/5 text-text-muted">
                                {{ form.unit }}
                             </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="flex items-baseline gap-2">
                                <span class="text-xl font-bold text-text-main">{{ item.value }} {{ item.unit }}</span>
                            </div>
                            <div class="text-sm font-medium text-text-muted">
                                {{ new Date(item.measured_at).toLocaleDateString(undefined, { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' }) }}
                            </div>
                             <div v-if="item.notes" class="mt-1 text-xs italic text-text-muted/70">
                                {{ item.notes }}
                            </div>
                        </div>
                        <button
