    ...
    <AuthenticatedLayout page-title="Measurements">
        ...
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold text-text-main">Measurements</h2>
        ...
        <div class="space-y-6">
            ...
            <GlassCard v-if="showAddForm" class="animate-slide-up">
                <h3 class="mb-4 font-semibold text-text-main">New Measurement</h3>
                <form @submit.prevent="submit" class="space-y-4">
                    <div>
                        <label class="mb-1 block text-sm font-medium text-text-muted">Body Part</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        />
                         <div class="space-y-1">
                            <label class="block text-sm font-medium text-text-muted">Unit</label>
                            <select
                                v-model="form.unit"
                                class="w-full rounded-xl border-white/10 bg-white/5 text-text-main focus:border-primary-500 focus:ring-primary-500"
                            >
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                >
                    <GlassCard class="h-full transition hover:scale-[1.02] hover:bg-white/10">
                        <div class="flex items-start justify-between">
                            <div>
                                <h3 class="font-bold text-text-main">{{ item.part }}</h3>
                                <div class="mt-1 text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">
                                    {{ item.current }} <span class="text-sm text-text-muted">{{ item.unit }}</span>
                                </div>
                                <div class="mt-1 text-xs text-text-muted">
                                    {{ new Date(item.date).toLocaleDateString() }}
                                </div>
                            </div>
                            <div v-if="item.diff !== 0" :class="[
                ...
                <div v-if="latestMeasurements.length === 0 && !showAddForm" class="col-span-full py-12 text-center">
                    ...
                    <GlassButton @click="showAddForm = true" class="mt-4">
                        Start
                    </GlassButton>
                </div>
            </div>
