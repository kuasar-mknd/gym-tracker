...
test('user can manage exercises', function (): void {
    ...
    $this->browse(function (Browser $browser) use ($user): void {
        ...
        $browser->pause(2000)
            ->waitForText('Dusk Test Exercise', 60);

        // 4. Edit the exercise
        $browser->waitFor('[data-testid="edit-exercise-button"]', 60)
            ->script("document.querySelector('[data-testid=\"edit-exercise-button\"]').click();");

        $browser->waitFor('input[type="text"]', 60)
            ->pause(1000)
        ...
        // 5. Verify update
        $browser->pause(2000)
            ->waitForText('Updated Exercise', 60);

        // 6. Delete the exercise
        $browser->waitFor('[data-testid="delete-exercise-button"]', 60)
