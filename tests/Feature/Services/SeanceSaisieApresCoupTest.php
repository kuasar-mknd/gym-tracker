...
function ligneAvecSerie(User $user, Exercise $exercice, string $quand, float $poids): WorkoutLine
{
    ...
    Set::factory()->create(['workout_line_id' => $workoutLine->id, 'weight' => $poids, 'reps' => 8, 'is_completed' => true]);
...
it('recommande d’après la séance la plus récente, pas la dernière saisie', function (): void {
    ...
    $workoutLine = WorkoutLine::factory()->create(['workout_id' => $courante->id, 'exercise_id' => $exercice->id]);
     ...
     */
    Cache::flush();
    $unitaire = $service->getRecommendedValues(WorkoutLine::with('workout')->findOrFail($workoutLine->id));

    Cache::flush();
    $lot = $service->batchRecommendedValues(WorkoutLine::whereKey($workoutLine->id)->with('workout')->get(), $user->id);
