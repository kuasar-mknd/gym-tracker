 ...
 *
 * `Workout::recalculerLeVolume()` filtre `is_completed` depuis #1499 : la meme
...
it('ne fait pas un record d’une série jamais cochée', function (): void {
    [$user, $exercice, $workoutLine] = ligneDeSeance();

    Set::factory()->create(['workout_line_id' => $workoutLine->id, 'weight' => 100, 'reps' => 5, 'is_warmup' => false, 'is_completed' => true]);
    Set::factory()->create(['workout_line_id' => $workoutLine->id, 'weight' => 200, 'reps' => 5, 'is_warmup' => false, 'is_completed' => false]);
...
it('accorde l’objectif de poids au record', function (): void {
    ...
    Set::factory()->create(['workout_line_id' => $workoutLine->id, 'weight' => 100, 'reps' => 5, 'is_warmup' => false, 'is_completed' => true]);
    Set::factory()->create(['workout_line_id' => $workoutLine->id, 'weight' => 150, 'reps' => 5, 'is_warmup' => true, 'is_completed' => true]);
    Set::factory()->create(['workout_line_id' => $workoutLine->id, 'weight' => 200, 'reps' => 5, 'is_warmup' => false, 'is_completed' => false]);
...
it('ignore les séries non faites dans un objectif de volume', function (): void {
    ...
    Set::factory()->create(['workout_line_id' => $workoutLine->id, 'weight' => 50, 'reps' => 10, 'is_warmup' => false, 'is_completed' => true]);
    Set::factory()->create(['workout_line_id' => $workoutLine->id, 'weight' => 100, 'reps' => 10, 'is_warmup' => false, 'is_completed' => false]);
...
it('compte dans le volume une série postée sans le champ', function (): void {
    [$user, , $workoutLine] = ligneDeSeance();
    ...
    $seance = Workout::findOrFail($workoutLine->workout_id);
...
it('respecte un `is_completed` explicitement faux', function (): void {
    ...
    expect((float) Workout::findOrFail($workoutLine->workout_id)->workout_volume)->toBe(0.0);
});
