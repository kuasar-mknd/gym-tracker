 ...
 *
 * Elle n'y est pas recopiée pour autant : le HTML est GÉNÉRÉ depuis la feuille,
 * et chaque contraste y est recalculé à la génération. Une page figée aurait
 * menti dès le premier jeton modifié — exactement la divergence que toute cette
 * charte existe pour supprimer, et il aurait été singulier de la réintroduire
...
class PublierLaCharte extends Command
{
    ...
    public function handle(): int
    {
        ...
        file_put_contents($chemin, self::page());
     ...
     *
     * L'ordre n'est pas alphabétique à dessein : on descend du plus général —
     * ce sur quoi tout se pose — au plus particulier. Un jeton qui n'entre dans
     * aucune famille apparaît quand même, sous « Autres » : une page de
     * documentation qui tait ce qu'elle ne sait pas classer est pire qu'inutile.
     *
     * @return list<array{0: string, 1: string, 2: callable(string): bool}>
     */
    private static function familles(): array
    {
        return [
            ['Surfaces', 'Ce sur quoi tout se pose.',
                static fn (string $n): bool => str_starts_with($n, 'surface-')],
            ['Traits et ombres', "Ce qui délimite, et les deux noirs qui ne sont pas de l'encre.",
                static fn (string $n): bool => str_starts_with($n, 'border') || str_starts_with($n, 'shadow-')],
            ['Texte', "Ce qu'on écrit, et sur quoi.",
                static fn (string $n): bool => str_starts_with($n, 'text-')],
            ['Accents vifs', "L'identité. Fonds, icônes, anneaux, lueurs — jamais les mots.",
                static fn (string $n): bool => str_starts_with($n, 'accent-')
                    && ! str_ends_with($n, '-deep') && ! str_ends_with($n, '-fill')],
            ['Variantes lisibles',
                "Les mêmes, calculées contre le pire fond qu'elles rencontrent : un lavis de leur propre accent à 20 %. Elles n'existent que pour porter du texte.",
                static fn (string $n): bool => str_ends_with($n, '-deep')],
            ['Surfaces pleines', 'Lues uniquement par les utilitaires appariés, jamais par un composant.',
                static fn (string $n): bool => str_ends_with($n, '-fill')],
            ['Tendance', 'Une direction, pas un jugement.',
                static fn (string $n): bool => str_starts_with($n, 'trend-')],
            ['Séance en cours', 'Le bandeau du tableau de bord, et lui seul.',
                static fn (string $n): bool => str_starts_with($n, 'session-')],
            ['Catégories', 'Huit groupes musculaires. Aucune intention — seulement se distinguer, ce qui se mesure en CIELAB.',
                static fn (string $n): bool => str_starts_with($n, 'category-')],
            ['Palette utilisateur', 'Seize teintes pour les habitudes. Chacune porte du blanc, par construction.',
                static fn (string $n): bool => str_starts_with($n, 'palette-')],
            ['Disques de fonte', 'Le code olympique. Une norme extérieure, pas un choix de charte.',
                static fn (string $n): bool => str_starts_with($n, 'plate-')],
        ];
    }
    ...
    {
        $jetons = Charte::couleursPleines();
        $restants = $jetons;
        $sections = '';

        foreach (self::familles() as [$titre, $sous, $appartient]) {
            $membres = array_filter($jetons, $appartient, ARRAY_FILTER_USE_KEY);
            ...
            $sections .= self::section($titre, $sous, $membres);
        }

        if ($restants !== []) {
            $sections .= self::section('Autres', "Aucune famille ne les réclame — c'est en soi une information.", $restants);
        }

        return self::gabarit($sections, self::tableau(), count($jetons), count(Charte::surfacesAppariees()));
    }

    /**
     * @param  array<string, string>  $membres
     */
    private static function section(string $titre, string $sous, array $membres): string
    {
        $pastilles = '';

        foreach ($membres as $nom => $valeur) {
            $pastilles .= self::pastille($nom, $valeur);
        }

        return sprintf(
            '<section class="famille"><h3>%s</h3><p class="sous">%s</p><div class="grille">%s</div></section>',
            e($titre),
            e($sous),
            $pastilles
        );
    ...
    /**
     * Une pastille, avec le meilleur texte que la couleur peut porter.
     */
    private static function pastille(string $nom, string $valeur): string
    {
        $encre = Charte::jeton('text-main');
        $surBlanc = Charte::contraste('#ffffff', $valeur);
        $surEncre = Charte::contraste($encre, $valeur);

        $porte = $surBlanc >= $surEncre ? 'blanc' : 'encre';

        return sprintf(
            '<div class="jeton"><div class="puce" style="background:%s"><span style="color:%s">Aa</span></div>'
            .'<div class="jeton-txt"><code>%s</code><span class="hex">%s</span><span class="mesure">%s · %.1f:1</span></div></div>',
            e($valeur),
            e($porte === 'blanc' ? '#ffffff' : $encre),
            e($nom),
            e($valeur),
            $porte,
            max($surBlanc, $surEncre)
        );
    }

    /**
     * Le tableau des surfaces appariées.
     */
    private static function tableau(): string
    {
        $lignes = '';
        ...
        foreach (Charte::surfacesAppariees() as $nom => $paire) {
            ...
            $lignes .= sprintf(
                '<tr><td><code>%s</code></td><td><span class="apercu" style="background:%s;color:%s">Terminer</span></td>'
                .'<td><code class="petit">%s</code></td><td><code class="petit">%s</code></td><td class="num ok">%.2f:1</td></tr>',
                e($nom),
                e($fond),
                e($texte),
                e($paire['fond']),
                e($paire['texte']),
                Charte::contraste($texte, $fond)
            );
        }
    ...
    private static function gabarit(string $sections, string $lignes, int $jetons, int $apparies): string
    {
        $fichiers = glob(base_path('tests/Feature/Conventions/*.php'));
        $gardes = $fichiers === false ? 0 : count($fichiers);
