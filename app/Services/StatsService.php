...
{
    /**
     * @return array<int, array{date: string, full_date: string, name: mixed, volume: float}>
     */
    ...
    public function getMonthlyVolumeHistory(User $user, int $months = 6): array
    {
        return \Illuminate\Support\Facades\Cache::remember(
            ...
            function () use ($user, $months): array {
                ...
                $grouped = $data->groupBy(fn (\stdClass $row): string => Carbon::parse($row->started_at)->format('Y-m'));
    ...
    {
        /** @var array<int, array{date: string, volume: float, name: string}> $history */
    ...
    /**
     * @return array{date: string, full_date: string, name: mixed, volume: float}
     */
