...
class StatsService
{
    ...
    {
        /** @var \Illuminate\Support\Collection<int, array{date: string, volume: float, name: string}> $history */
    ...
    /**
     * Fetch duration distribution buckets from the database.
     */
    protected function fetchDurationDistributionBuckets(User $user, int $days): \stdClass
    {
        ...
        $results = Workout::where('user_id', $user->id)
            ...
            ")
            ->toBase()
            ->first() ?? (object) ['b1' => 0, 'b2' => 0, 'b3' => 0, 'b4' => 0];
